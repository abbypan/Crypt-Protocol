...
my @test_psk = ( [ undef, undef ], [ 'test_psk', 0 ], [ 'test_psk', 1 ] , [ 'test_psk', 2 ], [ 'test_psk', 3 ], [ 'test_psk', 4 ]);

for my $pattern_name ( qw/X1X X1X1/ ) {
  ...
  ### $a_msg2_src
  my ( $a_msg2) = write_message( $noise_conf, $a_hs, [], $a_msg2_src );
  ### a_msg2: unpack( "H*", $a_msg2 )
  ### $a_hs

  # b read message from a
  ### b recv msg from a
  my ( $b_recv_a_msg2_r ) = read_message( $noise_conf, $b_hs, [], $a_msg2 );
  ### b_recv_a_msg2: $b_recv_a_msg2_r->[0]
  ### $b_hs
  
  # b write message to a
  ### b send msg to a
  my ( $b_msg2, $b_c1, $b_c2 ) = write_message( $noise_conf, $b_hs, [], undef);
  ### b_msg2: unpack( "H*", $b_msg2 )
  ### $b_hs
  ### $b_c1
  ### $b_c2

  # a read message from b
  ### a recv msg from b
  my ( $a_recv_b_msg2_r, $a_c1, $a_c2 ) = read_message( $noise_conf, $a_hs, [], $b_msg2 );
  ### a_recv_b_msg2: $a_recv_b_msg2_r->[0]
  ### $a_hs
  ### $a_c1
  ### $a_c2
